...
A **Fly Postgres** cluster runs in `lhr`, attached to the app via `flyctl postgres attach` (injects the connection string as an env var). Private-network connection — no public-internet hop. App-side connection pooling via `pg-pool` with pool size committed after Phase 1 memory measurement (expected 5–10 for the smallest machine class).
...
Migrations run via Fly's release_command pattern: `flyctl deploy --release-command "pnpm drizzle-kit migrate"` ensures migrations execute before the new release accepts traffic.
