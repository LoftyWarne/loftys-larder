...
- **Chosen:** GitHub Actions deploy workflow runs `flyctl deploy`; the `release_command` is configured in `fly.toml` (there is no `flyctl deploy --release-command` CLI flag) and runs `node /app/migrate.js && node /app/seed-reference.js` — a bundled drizzle-orm programmatic migrator (no `drizzle-kit` in the runtime image, DEC-61) followed by the reference-data seed. Both execute before the new release accepts traffic.
- **Reference seed step:** `seed-reference.js` (a third esbuild entrypoint, copied into the runtime image alongside `migrate.js`) runs `runReferenceSeeds` → `seedReference`, populating the global lookup tables (`units_of_measurement`, `preparation_types`, `ingredient_categories`, `meal_occasions`). These have no edit UI, so the release command is how they reach prod. The step is idempotent (`INSERT … ON CONFLICT (name) DO NOTHING`) and purely additive — it never updates or deletes rows, in the reference tables or any other table, so existing prod data is retained. `&&` ordering means it runs only if migration succeeds. Single source of truth stays in `backend/src/db/seeds/reference.ts`; new values are added there and seed on the next deploy. Note the trade-off: because it only inserts *missing* names, it cannot rename or remove an existing reference value — that needs a deliberate data migration. The household row is **not** seeded here (it predates this and lives in `runSeeds`, not `runReferenceSeeds`).
...
- **Note (2026-07-05):** Same dev/prod asymmetry surfaced the pre-paint theme-guard inline script in `frontend/index.html` being blocked by strict `script-src 'self'` (a latent issue since the theme feature shipped — non-fatal, only symptom is a possible dark-mode flash). Kept `script-src` strict per this DEC (no `'unsafe-inline'`) and allowed the one known script by its sha256 hash instead. Hash is pinned by a `security.test.ts` guard that derives it from the source file, so an edit to the script (or a formatter reflow of the HTML) that isn't matched by a hash update fails the build rather than silently regressing in prod. Vite does not currently minify inline HTML scripts, so source bytes == served bytes.
- **Revisit when:** Adding a new cross-origin source; tightening `style-src` away from `unsafe-inline` if currently permitted; the theme-guard script changes (update the hash — the guard test will tell you).
...
- **Update (FEAT-15, 2026-06-03):** The deferred call has been made. FEAT-15's sign-in form is the first runtime import from `/shared` (`signInSchema` in `shared/src/schemas/auth.ts`). Option (2) won: `frontend/vite.config.ts` and `frontend/vitest.config.ts` both alias `@loftys-larder/shared` → `../shared/src/index.ts`. `shared/package.json` gains a real `zod` dependency. `shared/tsconfig.json` keeps `noEmit: true` — no `dist` build is written. Option (2) was preferred over (3) (`exports → src`) so a published-library `dist` build can be added later without rewriting consumers, and over (1) (`tsc` build step) to avoid build orchestration ahead of need. The **backend** still has no runtime import from `/shared`; when that lands, `tsx watch` + `esbuild` bundle both resolve the workspace symlink to `shared/src/index.ts` directly. If anything trips, fall back to a `tsconfig` `paths` mapping in `backend/tsconfig.json` mirroring the frontend's.

---

### DEC-81 — `@typescript-eslint/only-throw-error` disabled for `frontend/src/routes/**`

- **Chosen:** The strict-type-checked `@typescript-eslint/only-throw-error` rule is turned off via an override block in `eslint.config.js` for files matching `frontend/src/routes/**/*.{ts,tsx}`. Everywhere else in the repo the rule remains on (inherited from `tseslint.configs.strictTypeChecked`).
- **Alternatives:** (a) Per-line `// eslint-disable-next-line @typescript-eslint/only-throw-error` above each `throw redirect(...)` call. Multiplies as `beforeLoad` guards land on every protected route. (b) Configure the rule's `allow` option with `{ from: 'package', name: 'Redirect', package: '@tanstack/react-router' }`. Tried during FEAT-15; did not match. `redirect()`'s return type is the framework's internal `Redirect` shape (a Response-like object), not a class symbol the rule's import-resolver can hook onto. (c) Restructure to not throw. Impossible — TanStack Router's `beforeLoad` contract IS `throw redirect(...)`; there is no return-based redirect path.
- **Why it won:** The framework's documented control-flow shape can't be modelled by the rule today. The surface is small and visually self-evident (route files are short, almost always `throw redirect(...)`). The rule continues to protect every other file in the repo — domain code, helpers, plugins, procedures — where throwing a non-Error would be a real bug.
- **Consequences (+):** Zero per-line suppressions to add and maintain. Future `beforeLoad` guards on `_authed/**` children inherit the relaxation automatically.
- **Consequences (−):** Route files can throw any value silently. Acceptable because route files are short and the throw-pattern is almost always `redirect(...)`; any real error-shape regression at the route layer would surface as a downstream `TRPCError` / test failure / runtime crash long before the lint rule would catch it.
...
- **Chosen:** Below `lg` (1024 px), `<RecipeBank>` is not rendered and its `recipes.list` infinite query is not started. Tapping any slot opens the existing slot-editor sheet (FEAT-31), which already supports flipping `slot_type` and picking a recipe via the existing `SearchableCombobox`. From `lg` upward the bank renders alongside the grid, paired with DnD (DEC-84).
- **Alternatives:** Hide the bank only below `md` (768 px) — keeps a three-tier model where small tablets in portrait get a bank-above-grid layout; collapse the bank into a sticky floating button that expands into a sheet; render a compact horizontal pill bar instead of the vertical bank.
- **Why it won:** The bank and the DnD machinery are layout-coupled. The side-by-side `lg:grid-cols-[16rem_1fr]` layout is the only viewport size where the bank fits beside the grid without crowding it; below `lg`, the bank-above-grid placement was already cramped and the DnD-on-a-top-bar gesture (drag down into the scrolling grid) is awkward. Anchoring the bank to the same breakpoint as DnD collapses three tiers into two and makes the rule "bank and DnD travel together" — a single mental model. The editor's combobox is already touch-friendly and is the existing path for *changing* a slot's recipe; using it as the sole assignment path on the compact branch removes a redundant interaction model on the tightest screens.
- **Consequences (+):** Two interaction shapes instead of three: simpler hook (`useIsLargeViewport` is a boolean), simpler tests, one rule covers the bank, DnD, and the layout. More grid real estate below `lg`. No Recipe Bank network round-trip on phones or small portrait tablets. Selection state (`selectedRecipe`) and the assignment-hint banner are only mounted alongside the bank.
- **Consequences (−):** Users below `lg` (phones + small portrait tablets) no longer get a recipe-list-glance view of "what could I cook" without first opening a slot — they have to commit to a slot before browsing. The bank-as-browser affordance is gone below `lg`. The recipe combobox in the editor needs to feel as good as the bank did; if typeahead is slow on a flaky connection, the trade is worse.
- **Revisit when:** Usability testing shows users repeatedly opening multiple slots just to scan recipes; or the cook reports wanting to compare candidates before committing; or a redesign decouples the bank from the side-by-side layout.
- **Cross-refs:** FEAT-40 (the implementation), FEAT-31 (the editor sheet this relies on, and the AC this narrows), DEC-53 (phone-first responsive — this is a phone-first refinement of FEAT-31, not a departure from DEC-53), DEC-84 (DnD shares the same `lg` gate; the bank and DnD are layout-coupled), DEC-49 (combobox primitive used by the editor).

### DEC-86 — PWA network-first runtime cache for the shopping-list query (one named bucket, 3 s timeout)

- **Chosen:** Register a service worker via `vite-plugin-pwa` (`registerType: 'autoUpdate'`, dev disabled). Workbox runtime caching has exactly one rule: `NetworkFirst` against `/\/api\/trpc\/shopping\.getForPlan/` with `networkTimeoutSeconds: 3`, a named bucket (`shopping-list-network-first`), `maxEntries: 32`, and a 7-day `maxAgeSeconds`. App-shell navigations fall back to the precached `/index.html`, but the `/api/*` namespace is on the navigate-fallback denylist so a tRPC fetch is never satisfied by the SPA HTML. No other read is cached.
- **Alternatives:** (a) `StaleWhileRevalidate` for the shopping list — would render instantly from cache even when online, but means the first paint after a write can be stale; for a list where users tap items in real time, "is this what the server thinks?" matters. (b) Cache the broader `shopping.*` namespace, or `getForPlan` plus future reads — broadens the surface and risks caching state the user expects to be live. (c) Skip the timeout (let `NetworkFirst` wait indefinitely) — on flaky networks the user stares at a spinner when a 3 s fallback would already have rendered the last-known list. (d) Network-only for everything; opt cached reads in per-feature later — defers the PWA work but blocks FEAT-43 (the offline mutation queue needs an SW already running).
- **Why it won:** `NetworkFirst` matches the household model: the server is the source of truth, and the offline-read affordance is a fallback, not the default. The 3 s timeout is aligned with the Fly cold-start budget (DEC-63 / DEC-64) — if the server hasn't responded by then, the user is on a path where cached-with-staleness beats blank-with-spinner. Naming the cache bucket lets FEAT-43 and ops inspect or evict it independently of the precache. Limiting the rule to `shopping.getForPlan` keeps the cache surface tight, honours the cross-cutting #16 invariant (don't change the tRPC URL shape; match the procedure segment, not the query string), and avoids accidentally caching mutations or unrelated reads.
- **Consequences (+):** Killing connectivity and reloading still renders the last-fetched shopping list. The SW + manifest also unlock "install app" on supporting browsers and add-to-home-screen on iOS. Unblocks FEAT-43 (offline mutation queue depends on an SW already shipping). Cache rule is colocated with the URL pattern it depends on (`src/lib/pwa-config.ts`) and unit-tested without invoking a full `vite build`.
...
### DEC-90 — Leftovers slots name their source; a plan-meal leftover is an `eat` item that draws the base pool

- **Chosen:** A `leftovers` slot records *what* is being eaten via a new `leftovers_source` enum (`plan_meal | takeaway | other`) on `meal_plan_slots`, coupled to the status by a CHECK: `slot_type = 'leftovers'` **iff** `leftovers_source IS NOT NULL`. A **`plan_meal`** leftover reuses the DEC-89 item model — it is the slot's single `eat` item, FK'd to the source recipe — so it draws the base consumption pool down (DEC-88) with no new mechanism, shows the dish name, and ties into the shortfall/"left in plan" math. **`takeaway`** / **`other`** are bare markers (no items). The editor offers earlier *Cooking*-slot dishes (deduped by recipe, `(date, occasion)`-ordered to match the consumption walk) plus Takeaway and Other; the dropdown is the only constraint on "earlier in plan" — the backend validates recipe-in-household and the single-eat-item shape but does **not** hard-enforce ordering (LWW, DEC-36). `other` is a bare marker, not free text (all user text is plain and this needs none).
- **Shopping exclusion:** the buy-list aggregation now scopes `eat`-item contributions to `slot_type = 'recipe'` (`selectMealRecipeContributions` and the eat branch of `selectIngredientContributions`). Without this, a plan-meal leftover's `eat` item would re-add the source recipe's ingredients — double-buying for a meal already provisioned at its Cooking slot. Consumption (base drawdown) is slot-type-agnostic and still counts the leftover; only provisioning is scoped.
- **Amends:** DEC-25 (the `slot_type` enum is unchanged, but `leftovers` now carries the coupled `leftovers_source`) and DEC-89 (a leftovers slot may hold exactly one `eat` item, where before `eat` items implied `slot_type='recipe'`). Builds on DEC-88 (the leftover draws the same 1:1 pool).
- **Alternatives:** (a) Free-text "what are you eating" — loses the FK, so no dish name, no consumption tie-in, and invites rich-text creep (DEC-49). (b) A dummy "Leftovers" recipe — rejected by DEC-25. (c) A separate `leftovers_of_slot_id` FK to the *source slot* rather than the recipe — more precise about which batch, but snapshot-like and brittle under edits (DEC-22); the recipe FK + derived pool is enough. (d) Leftover adds nothing to consumption (pure marker even for a plan meal) — then "left in plan" overstates supply; the whole point is that eating leftovers draws the batch down.
- **Why it won:** Reuses the existing `eat`-item machinery, so base drawdown, shortfall, and "left in plan" work for free; the enum + CHECK keep the three leftover shapes honest at the DB; the shopping scope is the one surgical guard needed to stop double-buying.
- **Consequences (+):** A leftover of a batch-cooked meal correctly depletes the pool and surfaces a shortfall when you plan to eat more than was cooked (worded around the *meal* for a non-base dish); takeaway/other stay cheap markers; no new table.
- **Consequences (−):** A schema migration straight to `main` (DEC-40) — hand-edited to backfill any pre-existing bare `leftovers` rows to `'other'` before the CHECK validates them. Plant-points still counts a leftover `eat` item as a plant eaten that day (deliberate — you did eat it; plan-level uses COUNT DISTINCT so no inflation). The "earlier in plan" rule lives only in the frontend dropdown, not the backend.
- **Revisit when:** A leftover needs to name the *specific* source batch (introduce alternative (c)'s slot FK), or "earlier in plan" needs server-side enforcement.
- **Cross-refs:** DEC-25 (amended), DEC-88, DEC-89 (amended), DEC-22, DEC-36, DEC-40, DEC-49.

### DEC-91 — Per-dish `prepared` / `eaten` quantities replace the single `servings` + `kind` split

- **Chosen:** Every `meal_plan_slot_items` row carries two quantities — **`prepared`** (portions cooked) and **`eaten`** (portions consumed at this occasion) — replacing the single `servings` smallint and **retiring the `kind` enum** (`eat | cook_ahead`). Both are `smallint >= 0` with `prepared + eaten > 0` (CHECK). A dish's *role* is now derived from its quantities rather than stored: it **produces** surplus when `prepared > eaten`, it **consumes** when `eaten > 0`. A pure cook-ahead batch is `prepared=N, eaten=0`; a normal dish is `prepared=eaten`; a leftovers/eat-out row is `prepared=0, eaten=N`. The base-vs-variation distinction stays entirely on the recipe (`is_base` / `base_recipe_id`), independent of quantity.
- **Consumption balance (generalises DEC-88):** the running cook-before-eat pool is now keyed by **every recipe id**, not just bases. Walking slots in `(date, occasion)` order, each slot first **produces** (`balance[recipeId] += prepared` for every item), then **consumes**: a *variation* draws its base pool by its own `prepared` (`balance[baseRecipeId] -= prepared` — making N variation portions needs N base portions), and every item draws its own pool by `eaten` (`balance[recipeId] -= eaten`). A draw below zero is the slot's shortfall. End-of-plan `balance` per recipe is "left in plan" — now meaningful for standalone meals and variations, not only bases. So over-cooking a **standalone** or a **variation** (case b) and eating the surplus at a later leftovers slot works with no new mechanism, and eating some of a cook-ahead base on the day (case a) is one row (`prepared=6, eaten=4`) instead of a confusing cook_ahead + eat pair of the same recipe.
- **Shopping (simplifies DEC-89/DEC-90):** the buy list is a **single scan over items with `prepared > 0`, scaled by `prepared`** (`quantity * prepared / baseServings`), each recipe contributing only its own `recipe_ingredients` rows. This collapses the former two-path union (`selectMealRecipeContributions` eat-path + `selectCooksBaseContributions` cook_ahead-path) into one, and **makes DEC-90's `slot_type='recipe'` shopping-exclusion redundant**: a leftovers/eat-out/takeaway consumption row has `prepared=0`, so it contributes nothing without any slot-type guard. It also removes the old latent double-buy when a base was both `cook_ahead` and eaten directly.
- **App-layer item rules (replace the `kind`-based ones in DEC-89):** a `recipe` slot needs ≥1 item with `eaten > 0`; `empty`/`eat_out`/`takeaway` allow no item with `eaten > 0` (prepared-only cook-ahead rows are still allowed on any slot type); a `plan_meal` leftovers slot holds exactly one pure-consume item (`prepared=0, eaten>0`); `takeaway`/`other` leftovers hold none. The "a cook-ahead must be a base" rule is **dropped** — any recipe may be over-cooked now.
- **Plant points (FEAT-41) now count only what's *eaten*:** the day/plan plant-point union keys its eat + serving-variation-base sources on `eaten > 0` and **drops the former cook-ahead source**. A dish contributes its plants only once it's eaten (`eaten > 0`); a batch cooked purely for later no longer counts on the cooking day — its plants land on the day/slot it is actually eaten (a later meal or leftovers slot, which carries `eaten > 0`). An eaten variation still pulls its base's plants via the traversal source. This changes the pre-DEC-91 behaviour, where a cooked base counted on its cook day regardless of consumption.
- **Supersedes/amends:** amends **DEC-88** (pool keyed by every recipe, variation base-draw by `prepared` not `eaten`), **DEC-89** (item shape is `{prepared, eaten}` not `{servings, kind}`; `kind` retired; the eat/cook_ahead contribution split collapses to one prepared-scaled scan), and **DEC-90** (the shopping-exclusion scope is now redundant and removed; leftovers still draw the pool via `eaten`).
- **Alternatives:** (a) Keep `kind` and add a second nullable quantity — leaves two overlapping surplus concepts and keeps the base-only limitation. (b) Per-serving conversion ratios between variation and base — more precise but a schema/UX cost the household hasn't asked for (still 1:1). (c) A `source_slot_id` FK so leftovers name the exact batch — snapshot-like and brittle (DEC-22); the per-recipe pool is enough. (d) Keep surplus as a separate cook_ahead row — the case (a)/(b) awkwardness and the latent double-buy are exactly what this removes.
- **Why it won:** one uniform per-dish shape expresses cook-more-than-you-eat for *every* recipe, closes the two real gaps, and makes both downstream consumers simpler — shopping becomes a single `prepared`-scaled scan and the balance is one produce-then-consume walk. `prepared`-driven provisioning matches "buy for what you cook," and `eaten`-driven consumption matches "the pool drains as you eat."
- **Consequences (+):** standalone/variation leftovers are first-class; the "(N left in plan)" hint generalises; shopping loses a whole contribution path and a slot-type special-case; the same numbers drive the editor, the card, the balance, and the buy list.
- **Consequences (−):** a schema migration straight to `main` (DEC-40) with a data backfill (`eat → prepared=eaten=servings`; `cook_ahead → prepared=servings, eaten=0`) before dropping `servings` + the `slot_item_kind` enum; every item consumer (schemas, procedures, shopping, balance, planner UI, tests) changes at once; the 1:1 base⇄variation assumption from DEC-88 remains (now applied to `prepared`).
- **Revisit when:** a real recipe needs a non-1:1 base⇄variation ratio, or leftovers need to name a specific source batch.
- **Cross-refs:** DEC-88 (amended), DEC-89 (amended), DEC-90 (amended), DEC-18, DEC-19, DEC-22, DEC-36, DEC-40; FEAT-30, FEAT-31, FEAT-32, FEAT-36.

### DEC-92 — The `/shopping` index defaults to the imminent upcoming plan, not merely the active one

- **Chosen:** The bare `/shopping` route resolves to a plan by precedence: **(1)** the soonest **future** plan whose `startDate` falls within a 2-day shop horizon (`startDate <= todayInLondon() + 2`), else **(2)** the **active** plan (today within `[startDate, endDate]`), else **(3)** the soonest future plan regardless of horizon, else **(4)** a "no current or upcoming plan" empty state. Resolution is client-side in `frontend/src/routes/-components/shopping-index-page.tsx` over two bounded `plans.list` reads (`status: 'active'` + `status: 'future'`); status is derived from the item's `startDate`/`endDate` against `todayInLondon()` since `planListItemSchema` carries no status field. The horizon uses a new `addCivilDays` helper in the frontend `date-utils`. Once on a list, a **segmented-tab picker** (`frontend/src/components/shopping/plan-picker.tsx`) in the shopping-page header lets the cook switch between the live lists: one tab per active + future plan (past plans excluded — reached via `/plans`), labelled by compact date range (`formatShortDayRangeLabel`), the shown plan marked `aria-current="page"`. It hides itself below two live plans, so the common single-plan case shows nothing. The auto-selection sets the default landing; the picker only makes switching between current and upcoming visible.
- **Why it won:** the household's real workflow is a single online order placed **the night before a plan starts** (the same single-shop assumption behind DEC-37). On that eve the outgoing plan is still `active` by the date buckets, so a naive "default to the active plan" hands the cook the wrong list on exactly the night the shop happens. Anchoring the default on the *upcoming* plan within a short horizon matches when the shopping action actually occurs. The 2-day (rather than 1-day) window lets the order go in a night early without losing the default.
- **Alternatives:** (a) Default to the active plan (prior behaviour) — wrong list on shop night. (b) Always prefer any future plan over the active one — hijacks a mid-week top-up of the current list as soon as next week's plan is created. (c) A chooser whenever two non-past plans coexist — an extra tap on the dominant path to avoid a rare wrong guess; rejected for friction (the switcher already covers the exception). (d) 1-day horizon — correct only on the literal eve; too tight for ordering a night early. (e) Resolve server-side via a new "plan to shop for" field/procedure — larger surface than a client-side pick over data already fetched.
- **Consequences (+):** the common path (open the app the night before, get next week's list) needs zero interaction; no schema or lifecycle change; the shopping list stays strictly per-plan (a plan ending never "clears" — it's simply no longer the default). Two bounded reads avoid dragging in all past plans that `status: 'all'` would. The picker keeps the outgoing list one visible tap away during the overlap window.
- **Consequences (−):** the horizon is a hardcoded constant, not user-configurable; client-side status derivation duplicates the backend's bucketing logic (kept in sync via `todayInLondon`); the picker labels assume a small number of live plans (segmented tabs get wide beyond ~3 — acceptable since the current + upcoming window is realistically two, and it would fall back to a dropdown if that ever changed).
- **Revisit when:** the household moves to multiple shops per plan (interacts with DEC-37's `shop_date` upgrade path), or the 2-day horizon proves wrong in lived use.
- **Cross-refs:** DEC-33 (Europe/London civil dates), DEC-37 (single-shop assumption), DEC-82 (per-plan shopping list, hard-delete cascade); FEAT-27 (plan status buckets), FEAT-38, FEAT-39.
